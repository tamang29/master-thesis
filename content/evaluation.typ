...
The findings are grouped by theme rather than by feedback source so that recurring observations can be connected to the requirements from @req-chapter and the implementation areas from @impl-chapter.
...
Additional feedback came from educational use. Students used Apollon in teaching contexts such as the introductory course and iPraktikum,#footnote[TUM iPraktikum: #link("https://aet.cit.tum.de/projects/ipraktikum/")[aet.cit.tum.de/projects/ipraktikum].] modeling exercises, and project-related work. Instructors, tutors, and Artemis developers encountered the tool through exercise creation, assessment, quiz generation, collaboration, and integration work.

Later in the project, feedback sessions were used to inspect the implemented state of Apollon more directly. Users worked with Apollon on realistic tasks from teaching, research, supervision, or Artemis-related work. One participant with limited recent Apollon usage regularly used Lucidchart#footnote[Lucidchart product website: #link("https://www.lucidchart.com/pages/")[lucidchart.com/pages].] for UML diagrams and draw.io#footnote[draw.io documentation: #link("https://www.drawio.com/docs/getting-started/")[drawio.com/docs/getting-started].] for quick sketches. An activity-diagram task in that walkthrough contributed observations about editing friction, layout control, export behavior, collaboration awareness, sharing modes, standalone navigation and diagram management, and notation support to the same thematic feedback pool as the other sessions.

The collected feedback was heterogeneous. Some feedback described concrete bugs, such as broken assessment controls or export problems. Some feedback described workflow issues, such as the difficulty of creating straight edges or managing diagrams across local and shared contexts. Other feedback described future expectations, such as PlantUML#footnote[PlantUML project website: #link("https://plantuml.com/")[plantuml.com].] interoperability, review comments, offline support, stronger ownership of shared diagrams, and more complete UML notation support.
...
Feedback was processed in three steps. Reported issues, developer comments, and observed workflow problems were grouped into requirement areas such as edge usability, export correctness, collaboration awareness, standalone navigation, and educational integration. Implementation work addressed these areas in their relevant editor, application, or host context. Later feedback sessions and integration tests were used to inspect whether the implemented behavior supported the intended workflows and to identify remaining problems.
...
The implementation addressed part of this feedback through path manipulation, reconnection, adaptive controls, and crossing readability. Later walkthrough feedback reacted positively to line jumps because they made crossing edges more understandable without requiring manual rearrangement. Implementing the behavior in the shared editor also made it available to different host applications.

Later feedback identified easier straight-line correction, more flexible anchor behavior, larger selectable areas, and connection points that adapt more strongly to element size. The walkthrough also showed that routes around nearby elements could waste space and that very close elements could produce U-shaped artifacts instead of compact straight connections. Participants additionally requested manual edge-label placement, especially for decision labels such as yes/no in activity diagrams. These problems were addressed in follow-up implementation after the feedback: straightening behavior, anchor handling, selectable edge areas, adaptive connection controls, compact routing, and manual label placement were refined. The sequence demonstrates how formative feedback produced concrete implementation changes within the thesis period.
...
The feedback also confirmed that collaboration-awareness cues are session-specific rather than part of the persisted diagram content. This matches the architectural separation of awareness state from the model.
...
The implementation addressed export feedback through server-side and compatibility-oriented export paths, rendering fixes, and visual regression checks. These changes support the requirements from #section-link(<req-export-rendering>) and improve portability across application contexts.
...
The home-page and routing implementation responded to this feedback by providing a diagram-management entry point. The local and shared distinction was not immediately obvious in all cases, but became clearer after interaction with the interface.
...
Feedback from Artemis developers, teaching contexts, and user sessions showed that Apollon must support educational workflows beyond standalone diagram creation. Integration feedback therefore became a major source of follow-up requirements.

Integration feedback and checks covered continuity after the Artemis migration across modeling, quiz, assessment, and team-modeling workflows, including the focused Athena update for the earlier and current serialized model formats required by feedback generation. These observations relate to #section-link(<req-artemis>)#h(0.25em)and #section-link(<req-athena>).
...
The editor-side modernization described in #section-link(<impl-ui-modernization>) addressed part of this issue by replacing older interface layers with lighter reusable primitives and adapting related editor controls and tests.
...
The strongest concern was edge interaction. Its recurrence across sources shows that small geometric and selection details strongly affect the modeling flow; the follow-up implementation addressed the reported issues within the thesis period.
...
The additional walkthrough reinforced the existing pattern rather than introducing a separate evaluation result: participants recognized improvements in core workflows while still encountering friction in precise editing, discoverability, sharing, and notation support.

The findings clarify the role of Apollon in education. Feedback considered not only drawing interactions but also teaching, review, interoperability, export, and long-term ownership, supporting the thesis treatment of usability and integration as connected problems.
...
The feedback identifies several concrete future-work directions. For edge and layout behavior, the next step is systematic usability evaluation of the implemented refinements across diagram types and diagram sizes. Such evaluation should test whether straightening, label placement, selectable areas, adaptive connection controls, and compact routing remain predictable in realistic modeling tasks.

A second direction is stronger persistence, sharing, and ownership. Optional login, persistent shared diagrams, clearer or simpler sharing modes, local file-system integration, and Git-friendly workflows would make Apollon more suitable for scientific and long-term project work. These features would extend the standalone home page from a navigation surface into a stronger diagram management system.
